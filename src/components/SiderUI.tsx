...
import {
    AppstoreOutlined, MailOutlined, SettingOutlined, HomeOutlined,
    PartitionOutlined, TeamOutlined, DatabaseOutlined, GoldOutlined, BellOutlined
...
const SiderMenu = ({ UserAuthority }: { UserAuthority: number }) => {
    ...
    const items_asset: MenuProps["items"] = [    //资产管理员侧边栏
        ...
        getItem("外部应用", "4", <AppstoreOutlined />,
            // OuterAppList?.map((item, id) => 
            // {
            //     if (!item.IsLock) {
            //         return getItem(item.AppName, item.AppUrl);
            //     }
            // })
            OuterAppList?.filter(item => !item.IsLock).map((item,id)=>( getItem(item.AppName, item.AppUrl)))),
    ];
    const items_employee: MenuProps["items"] = [ //员工侧边栏
        getItem("首页", "1", <HomeOutlined />),
        getItem("消息列表", "2", <BellOutlined />),
        getItem("资产管理", "3", <GoldOutlined />),
        getItem("外部应用", "4", <AppstoreOutlined />, OuterAppList?.filter(item => !item.IsLock).map((item, id) => (getItem(item.AppName, item.AppUrl)))),
    ];
